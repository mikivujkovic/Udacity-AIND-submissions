...
function drawFeaturePoints(canvas, img, face) {
  ...
  for (var id in face.featurePoints) {
    ...
    // <your code here>
    ctx.beginPath();
    ctx.arc(featurePoint.x, featurePoint.y, 4, 0, 2 * Math.PI);
    ctx.stroke();
  }
}
...
function drawEmoji(canvas, img, face) {
  ...
  // <your code here>
  ctx.fillText(face.emojis.dominantEmoji, face.featurePoints[0].x-100, face.featurePoints[0].y-50);
}
...
var total = 0; // total guesses
var showInterval = 10; // how long is one emoji shown
var time = getTimeInSeconds(); // time to check if showInterval passed
...
// show random emoji from the list
...
function play(faces) {
  ...
  // if we matched the emoji increase correct, total, show the score and move to the next emoji
  if (target == toUnicode(faces[0].emojis.dominantEmoji)) {
      correct++;
      total++;
      showRandomEmoji();
      time = time + showInterval;
      setScore(correct, total);
  }

  // if we did not match emoji in given time, increase the total and move to the next emoji
  if (startTime >= time) {
    showRandomEmoji();
    total++;
    time = time + showInterval;
